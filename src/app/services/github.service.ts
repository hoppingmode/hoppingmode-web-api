...
export class GitHubApiClient {
  private RepositoryPortfolioTag = "portfolio-project";
  private GitHubReadmeEncoding: Encoding = "utf-8";
  private GitHubGqlApiBaseUrl = "https://api.github.com/graphql";
  private GitGubApiBaseUrl = "https://api.github.com";
  ...
  constructor() {
    ...
    this.gql = new GraphQLClient(this.GitHubGqlApiBaseUrl, {
      headers: {
  ...
  private listProject(repo: GitHubRepositoryDTO) {
    ...
    return (
      repo.topics.list.findIndex((t) => t.topicItem.name === this.RepositoryPortfolioTag) !== -1
    );
  }

  private constructProjectListDTO(repo: GitHubRepositoryDTO, isPinned: boolean): ProjectSummaryDTO {
    return {
      name: repo.name,
      description: repo.description,
      githubIsPinned: isPinned,
      githubUrl: repo.url
    };
  }
  ...
  public async constructProjectViewDTO(repoName: string): Promise<ProjectViewDTO> {
    try {
      ...
      if (githubReadme != null) {
        projectReadme = {
          content: Buffer.from(githubReadme.content, this.GitHubReadmeEncoding).toString("base64")
        };
      }
